...
exports.studentConfirmSessionId_PostInAttendence = async (req, resp) => {
    try {
        const { session_id, device_id, student_reg, student_sign, device_name } = req.body;
        // const checkCourse_id = await pool.query('SELECT EXISTS(SELECT course_id FROM timetable WHERE course_id =$1)', [course_id]);

        // if (checkCourse_id.rows[0].exists) {
        //// Now check if session_id id exist in sessions table
